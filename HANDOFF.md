...
> 最后更新：2026-06-18 ｜ 版本：v12 ｜ 单人开发者：胡佳仪
>
> v11 UI 大改(治愈系/清新，见第9节)；v12 每日挑战(`mode==="daily"`，日期种子=当天固定关卡)；v13 成就系统(`ACH` 10项，`stats`=cb_stats，已解锁=cb_ach，达成奖励金币+#achToast)。**粘性三件套全部完成。下一步：停止加功能，发朋友测留存。**
>
> ⚠️ **【已转型】** 朋友实测连锁版=红灯("不好玩、不会选这款")→ 按框架**换核心、复用工程**。**当前线上游戏 = 「再吹一点点」**(`merge-game/balloon.html` → 部署为 `index.html`)：气球 push-your-luck(情绪起伏)+ 永久升级商店(养成)。连锁版归档 `chain-archive.html`/`chain.html`。balloon 复用治愈UI/货币 cb_coins/每日签到/成就框架/部署。**正在发朋友测新核心是否"想玩下去"。** 教训：先验证核心 fun 再打磨外围。
...
## 9. UI 大改 — ✅ 已完成（v11）
**最终方向（用户定）**：**治愈系 + 清新**，不要"可爱"过头；**少 emoji**；**禁止蓝紫配色**(最 AI 味)。
**已落地配色**：米白/奶油底(`#f8f3ea→#efe5d4`)、暖棕字(`#5d5346`)、陶土橙主按钮(`#cf9277`)、鼠尾草绿(`#9bb072`，用于 +1 球/过关)、燕麦金(`#dcab4d`，金币/星)。小球=暖系自然马卡龙色(无蓝紫)。主题(多地图)=奶油/薄荷/蜜桃/沙/腮红 暖色渐变。
**去 emoji 全部落地**：特殊球=CSS 白十字 / 金色脉冲发光 / 绿球"+1"文字；障碍=陶土圆角方块；金币=CSS 小金币；开关=文字"音效/音乐/震动"；星=★；皮肤=糖霜/果冻/奶油哑光/柔光/自定义(均无 emoji)。胖糖果按钮(按下下沉)、奶白卡片+柔和阴影+大圆角。
> 若日后还要再调风格：颜色变量集中在 `:root`(--bg1/--ink/--accent/--sage/--gold)，小球色在 `COLORS`，主题在 `THEMES`，特殊球画法在 `spawnBubble`，皮肤在 `applySkin`。

<details><summary>(存档) 原始计划要点</summary>
铁律：只改视觉、逻辑不动、id 不变；逐个替换 emoji；统一奶油卡片风。已按此执行。
</details>
